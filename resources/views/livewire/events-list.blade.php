                            ...
                            <table class="min-w-full divide-y divide-neutral-200">
                                <thead>
                                    <tr class="text-neutral-500">
                                        ...
                                        <th class="px-2 py-3 text-xs font-medium text-left uppercase">Ministered By</th>
                                        <th class="px-3 py-3 text-xs font-medium text-left uppercase">Location</th>
                                        <th class="px-3 py-3 text-xs font-medium text-left uppercase">Report</th>
                                        <th class="px-3 py-3 text-xs font-medium text-center uppercase">Date</th>
                                        <th class="px-3 py-3 text-xs font-medium text-center uppercase">Time</th>
                                ...
                                <tbody class="divide-y divide-neutral-200">
                                    ...
                                    <tr class="text-neutral-800">
                                        ...
                                        <td class="px-3 py-4 text-sm font-medium whitespace-nowrap">
                                            <img src="{{asset('storage/events/'.$event->image_url)}}" class="object-cover h-10 w-10 rounded-full" alt="">
                                        </td>
                                        <td class="px-3 py-4 text-sm whitespace-nowrap">{{$event->name}}</td>
                                        <td class="px-2 py-4 text-sm whitespace-nowrap">{{$event->minister}}</td>
                                        <td class="px-3 py-4 text-sm whitespace-nowrap">{{$event->location}}</td>
                                        <td class="px-3 py-4 text-sm whitespace-nowrap">{{$event->report ? 'Yes' : 'No'}}</td>
                                        <td class="px-3 py-4 text-sm whitespace-nowrap">
                                            <div>
                                                <span>{{Carbon::make($event->start_date)->format('Y-m-d') }}</span> 
                                                <span>|</span><br>
                                                <span>{{Carbon::make($event->end_date)->format('Y-m-d') }}</span>
                                            </div>
                                        </td>
                                        <td class="px-3 py-4 text-sm whitespace-nowrap">
                                            <div>
                                                <span>{{Carbon::make($event->start_date)->format('g:i A') }}</span> 
                                                <span>|</span>
                                                <span>{{Carbon::make($event->end_date)->format('g:i A') }}</span>
                                            </div>
                                        </td>
