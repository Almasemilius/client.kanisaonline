                ...
                <div class="w-full md:px-14 md:grid md:grid-cols-9 md:gap-16">
                    <div class="w-full space-y-3 md:col-span-3 md:space-y-5">
                        <div class="bg-fading w-full h-56  flex items-center justify-center rounded-md overflow-hidden">
                            ...
                            @elseif ($image_url)
                            <img src="{{asset('storage/events/'.$event->image_url)}}" alt="" class="h-full w-full object-cover overflow-hidden">
                            @else
                    ...
                    <div class="col-span-6 w-full flex flex-col space-y-5">
                        <div class="">
                            <div class="church-input-group">
                                ...
                                <input wire:model='event.name' type="text" class="input-field-wizard bg-background h-10" placeholder="Enter Name">
                            </div>
                            <div class="error-div">
                                @error('event.name') <span class="error">{{ $message }}</span> @enderror
                            </div>
                        </div>
                        <div class="">
                            <div class="church-input-group">
                                <label for="contact_person" class=" input-label">Minister: <span class="text-primary">*</span></label>
                                <select wire:model='event.minister' name="yearSelect" id="yearSelect" class="input-field-wizard bg-background h-10">
                        ...
                        <div class="">
                            <div class="church-input-group">
                                ...
                                <textarea wire:model='event.description' name="denominationSelect" id="denominationSelect" class="textarea-field-wizard bg-background h-20">
                                </textarea>
                            </div>
                            <div class="error-div">
                                @error('event.description') <span class="error">{{ $message }}</span> @enderror
                            </div>
                        </div>
            ...
            <div x-show.transition.in="step === 2" class="w-10/12 ml-9 ">
                <div class="w-full grid grid-cols-2 md:px-14 gap-5 md:gap-10">
                    <div class="church-input-group col-span-2 md:col-span-1">
                        <label for="name" class="input-label">Start Date:</label>
                        <input wire:model='start_date' type="date" class="rounded-full w-full border-none  px-5 active:ring-0 focus:ring-0 text-black font-droidBold text-xs bg-background h-10">

                    </div>
                    <div class="church-input-group col-span-2 md:col-span-1">
                        <label for="name" class="input-label">End Date:</label>
                        <input wire:model='end_date' type="date" class="rounded-full w-full border-none  px-5 active:ring-0 focus:ring-0 text-black font-droidBold text-xs bg-background h-10">
                    </div>
                    <div class="church-input-group col-span-2 md:col-span-1">
                        <label for="name" class="input-label">Start Time:</label>
                        <input wire:model='start_time' type="time" class="rounded-full w-full border-none  px-5 active:ring-0 focus:ring-0 text-black font-droidBold text-xs bg-background h-10">
                    </div>
                    <div class="church-input-group col-span-2 md:col-span-1">
                        <label for="name" class="input-label">End Time:</label>
                        <input wire:model='end_time' type="time" class="rounded-full w-full border-none  px-5 active:ring-0 focus:ring-0 text-black font-droidBold text-xs bg-background h-10">
                    </div>
                    <div class="church-input-group col-span-2">
                        <label for="name" class=" input-label">Location:</label>
                        <input wire:model='event.location' type="text" class="rounded-full w-full border-none  px-5 active:ring-0 focus:ring-0 text-black font-droidBold text-xs bg-background h-10">
                    </div>

                    <div class=" w-full col-span-2 flex justify-end space-x-10 pt-5">
                        <a href="#" class="btn text-secondary border border-secondary">
                            Discard</a>
                        <button wire:click="addEvent" class="btn text-white bg-secondary">Finish</button>
