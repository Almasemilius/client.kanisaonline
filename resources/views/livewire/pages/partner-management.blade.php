<div class="space-y-5">
    ...
    <div class="w-full flex justify-between p-5 bg-white">
        <div class="flex items-center gap-5 ">
            <select wire:model.live="itemCount" name="" id="" class="rounded-md bg-gray-100 border-gray-200">
                <option value="6">6</option>
                <option value="12">12</option>
                <option value="24">24</option>
            </select>
            @if ($partners->count() > 0)
            <span>Showing {{ $partners->firstItem() }} to {{ $partners->lastItem() }} of {{ $partners->total() }} entries</span>
            @endif
        </div>
        <div>
            <a href="/add-partner" class=" primary-btn">
                ...
                <span class="breadcrumb-text">Add Partner</span>
            </a>
        </div>
    </div>
    <div>
        <div class="grid grid-cols-6 gap-5">
            ...
            <div class="w-full bg-white h-56 p-3 space-y-2">
                <div class="bg-cover h-40 w-full">
                    <img src="{{asset('storage/sponsors/'.$partner->logo_url)}}" class="object-cover" alt="">
                </div>
                <div class="w-full flex justify-between ">
                    <span class="text-sm">{{$partner->name}}</span>
                    <a href="{{route('add.partner', $partner->id)}}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="fill-secondary p-1 bg-orange-100 h-6 w-6 rounded-full fill-orange-700" viewBox="0 0 24 24">
            ...
            @empty
            <div class="col-span-6 w-full flex justify-center bg-white text-gray-500 p-10">
